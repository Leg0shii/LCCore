...
@Command(names = {"delete"}, permission = "tags.delete", desc = "<tag id/name>")
public class TagsDeleteCommand implements CommandClass {

    @Inject private TagManager tagManager;

    @Command(names = "")
    public void tagDelete(CommandSender sender, @ReflectiveTabComplete(clazz = TagManager.class, method = "getTagNames") String tagIdName) {
        Bukkit.getScheduler().runTaskAsynchronously(Linkcraft.getPlugin(), () -> {
            Tag tag = tagManager.getTag(tagIdName);

            if(tag == null) {
                MessageUtil.send(Message.TAGS_NO_TAG, sender, tagIdName);
                return;
            }
